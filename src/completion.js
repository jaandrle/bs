...
function resolveCompletions(completions, now) {
	let out = completions
		.filter((item) => item.toLowerCase().indexOf(now.toLowerCase()) !== -1)
		.sort();
	if (!now)
		out = out.map((f) =>
			f.includes("/") ? f.slice(0, f.indexOf("/") + 1) : f,
		);
	if (out.length)
		log(
			out.sort((a, b) => b.split("/").length - a.split("/").length).join(" "),
		);
	process.exit(0);
}

function completionBash(
	{ api, completionScript, ls },
	[level, now = "", prev, first, second, third],
) {
	level -= 2;
	...
	if (!level) {
		...
		else return resolveCompletions(ls(), now);
	}
	if (first === ".run") {
		if (level === 1)
	...
	if (first.startsWith(".")) return resolveCompletions(options_global, now);
	if (!ls().includes(first)) return process.exit(0);
	...
	try {
		...
		if (only_one && !subcommands[0].name)
			// cli supports only one subcommand
			level += 1;
		if (!level)
			return resolveCompletions(
				subcommands.flatMap((r) => r.command),
				now,
			);

		const subcommand = only_one
			? subcommands[0]
			: subcommands.find((r) => r.command === second);
		if (!subcommand || !subcommand.flags) return process.exit(0);
		return resolveCompletions(
			subcommand.flags.map((r) => r.name),
			now,
		);
	} catch (_) {
		log(
			"Completion script in .bsrc has probably invalid json scheme, see https://github.com/fvictorio/completely",
		);
