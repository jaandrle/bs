...
const name = package_name.startsWith("@")
	? package_name.slice(package_name.indexOf("/") + 1)
	: package_name;
const headline = name + ": Build system based on executables";

const fc = (code, ...rest) =>
	format(
		"%c" + (!Array.isArray(code) ? code : String.raw(code, ...rest)),
		css.code,
	); //format as code
chars["fc"] = 2;
const help = !process.argv.includes("--help")
	...
	: {
			...
			run:
				"Run the given build executable " +
				format("%c (default when `bs [script]`)", css.highlight),
			ls:
				"Lists all available executables" +
				format("%c (default when only `bs`)", css.highlight),
			mkdir: [
			...
			completion: [
				"Register a completions for the given shell",
