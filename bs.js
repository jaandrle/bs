...
const api = require("sade")(info.name)
	...
	.action((filter) => {
		...
		const list_final = filter
			? list.filter((x) => x.script.includes(filter))
			: list;
...
function readReadme(path_bs) {
	...
	const found = content
		.flatMap((line, i) =>
			line.match(new RegExp(`^#+ \`?(./)?${info.name}/(.*)`))
				? [[content[i], i]]
				: [],
		) //`
		.map(([l, i]) => {
...
function init(root = pwd()) {
	...
	if (!readme.exists) writeFileSync(readme.path, readme.content.join("\n"));
	log(
		"%cReadme%s:",
		css.highlight,
...
function cat(filter) {
	...
	lines.forEach(function echoLine(line) {
		if (line.trim().startsWith("#")) return log(`%c${line}`, css.headline);
		log(line);
	});
...
function ls({ is_out = false } = {}) {
	...
	return listExecutables(folder_root, 0)
		...
		.sort(({ script: a }, { script: b }) => {
			const deep = [a, b].reduce(
				(acc, curr, i) => acc + (-1) ** i * (curr.match(/\//g) || []).length,
				0,
			);
			if (deep) return deep;
...
function lsPrintNth({ script, docs }) {
	...
	const sep = ": ";
	const chars_start =
		chars["css.script"] + chars.fc + script.length + sep.length;
	if (docs)
		out += `${sep + docs.slice(0, columns - chars_start - 1 /* =… */)}…`;
...
function run(script) {
	...
	return spawn(
		...
		if (typeof exit_code === "number") return exit(exit_code);
...
function completion(shell) {
	const {
		completionBash,
		completionRegisterBash,
	} = require("./src/completion.js");
	if ("bash" === shell) return completionRegisterBash(info.name);
